...
6. [Oracles and smart contracts](<notebooks/6. oracles_and_smart_contracts.ipynb>) — contracts at addresses, calls in `Block`s, contract-to-contract reads, and oracle reports that change from block to block.
7. [Flash loans](<notebooks/7. flash_loans.ipynb>) — same contracts, first shoved with owned ETH, then a flash loan: atomic exploits, rollback, and included ≠ succeeded.
